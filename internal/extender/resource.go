...
func (s *SparkSchedulerExtender) selectDriverNode(
	...
	// Okay so now we have a situation where at least something fits
	...
	// We create the reservation with the partial solution either way
...
func (s *SparkSchedulerExtender) getNodesWithExecutorsBelongingToSameApp(ctx context.Context, executor *v1.Pod) map[string]bool {
	...
	if sr, ok := s.resourceReservationManager.GetSoftResourceReservation(appID); ok {
		for pod, reservation := range sr.Reservations {
			if pod != common.Driver {
				nodeNames[reservation.Node] = true
			}
		}
